	...
	for(uint32_t bus = 0; bus < 256; bus++)
    {
        for(uint32_t slot = 0; slot < 32; slot++)
        {
            for(uint32_t function = 0; function < 8; function++)
            {
                    ...
                    newline();
                    kprint("PCI: Found a device. ID: ", 0xFFFFFF);
                    kprint(to_hstring16(device), 0xFFFFFF);  
                    kprint(", vendor: ", 0xFFFFFF);
                    kprint(GetVendorName(vendor), 0xFFFFFF);
                    kprint(", className: ", 0xFFFFFF);
                    kprint(DeviceClasses[class], 0xFFFFFF);
                    kprint("  (class: 0x", 0xFFFFFF);
                    kprint(to_hstring16(class), 0xFFFFFF);
                    kprint(")", 0xFFFFFF);
...
void pci_init()
{
	...
	pci_probe();
    newline();
	kprint("PCI driver loaded.", 0xFFFFFF);
    newline();
}
...
void pci_proc_dump(uint8_t *buffer)
{
	for(int i = 0; i < devs; i++)
	{
		...
		if(pci_dev->driver)
			kprint(pci_dev->driver->name, 0xFFFFFF);
		else
			kprint(pci_dev->vendor, 0xFFFFFF);
	}
}
